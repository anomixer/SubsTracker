...
/**
 * 獲取所有訂閱（從新 repo 讀取）。
 * 會附帶 reminderRules / reminderRulesSummary，供列表展示真實提醒策略。
 *
 * @param {any} env
...
/**
 * 將多規則同步回訂閱上的 legacy 提醒欄位，避免列表/舊路徑與規則脫節。
 *
 * @param {any} env
 * @param {string} subId
...
async function syncLegacyReminderFields(env, subId, rules) {
  try {
    ...
    console.error('[subscriptions] 同步 legacy 提醒欄位失敗:', error);
  }
}
