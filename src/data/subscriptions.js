...
async function manualRenewSubscription(id, env, options = {}) {
  try {
    ...
    return { success: true, subscription: subscriptions[index], message: '續訂成功' };
  } catch (error) {
    console.error('手動續訂失敗:', error);
    return { success: false, message: '續訂失敗: ' + error.message };
  }
}
...
async function deletePaymentRecord(subscriptionId, paymentId, env) {
  try {
    ...
    if (paymentIndex === -1) {
      return { success: false, message: '支付記錄不存在' };
    }
    ...
    return { success: true, subscription: subscriptions[index], message: '支付記錄已刪除' };
  } catch (error) {
...
async function updatePaymentRecord(subscriptionId, paymentId, paymentData, env) {
  try {
    ...
    return { success: true, subscription: subscriptions[index], message: '支付記錄已更新' };
  } catch (error) {
